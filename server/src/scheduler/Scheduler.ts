...
export class Scheduler {
  ...
  constructor(maxAgeThreshold: number = 5) {
   ...
   */
  public selectNextJob(highQueue: QueueJob[], lowQueue: QueueJob[]): QueueJob | null {
    ...
    const starvedJobIndex = lowQueue.findIndex(job => job.age >= this.maxAgeThreshold);
   ...
   */
  public updateQueuePositions(highQueue: QueueJob[], lowQueue: QueueJob[]): void {
    let position = 1;
